...
<div class="container-fuild" style="background: linear-gradient( 90deg, #f65e38 0, #f68a39 51%, #f65e38);">
    <div class="container  pt-5 pb-5" style="width: 1260px;">
        <h4 class="text-light fw-bold">Đăng ký thành viên BOOKING - Tích điểm thưởng và nhận ưu đãi</h4>
        <span class="text-light pt-1">Nhanh chóng, tiện lợi và an toàn. Đăng ký liền tay, rinh ngay quyền
            lợi.</span>
    </div>
        ...
        <div class="col-4 ">
            <div class="account">
                <form class="account2 p-4  rounded-3 " method="post" style="background-color: #fff;">
                    <h3 class="account__title fw-bold fs-3">Đăng ký thành viên</h3>
                ...
                if (isset($_POST['add_user'])) {
                    ...
                    $result_user = $conn->query($getuser);
                    if($result_user->num_rows > 0) {
                        echo "
                            <script>
                                alert('email này đã được đăng kí, vui lòng đăng nhập hoặc đăng kí lại!!!')
                            </script>
                        ";
                    }else{
                        if ($password === $repassword) {
                            $password = md5($password);
                            $sql = "INSERT INTO `user`(`name`, `email`, `password`, `phone`,`avatar`,`time`) VALUES ('$name', '$email','$password','$phone','user.png','$today')";
                            $result = $conn->query($sql);
                            if ($result) {
                                setcookie('user', $email, time() + (86400 * 30), "/");
                                $sql_user1 = "SELECT * FROM user where email = '$email' LIMIT 1";
                                $result_user1 = $conn->query($sql_user1);
                                if($result_user1->num_rows > 0){
                                    while ($row_user1 = $result_user1->fetch_assoc()){
                                        $id_user1 = $row_user1['id'];
                                        $time_out = date('Y-m-d H:i:s',(strtotime($today)+10*86400));
                                        $permitted_chars = '0123456789abcdefghijklmnopqrstuvwxyz';
                                        $magiamgia =  substr(str_shuffle($permitted_chars), 0, 6);
                                        $sql_magiam = "INSERT INTO mgguser VALUES (null,$id_user1,1,'$magiamgia',200000,'$time_out','$today')";
                            ...
                            ">Chào Trương Mạnh Dũng,</h1>
                                                                <p class="near_title last" style="margin-top: 10px;margin-bottom: 0;">Bạn đã đăng kí thành công tài khoản</p>
                                                                <p class="near_title last" style="margin-top: 10px;margin-bottom: 0;">Bạn được nhận uy đãi mã giảm giá : <strong style="font-weight: bold; color: red">'.$magiamgia.'</strong></p>
                                                                <p class="near_title last" style="margin-top: 10px;margin-bottom: 0;">Thời gian sử dụng mã giảm giá đến ngày :'.$time_out.'</p>
